...
%% Capillary kernels G_S

[rts,ejs] = surfwave.capillary.find_roots(-1,1,-1);
[val,grad,hess] = surfwave.capillary.gphihelm(rts,ejs,src.r,targ.r);
val = reshape(val,size(X));
gradx = reshape(grad(:,:,1),size(X));
grady = reshape(grad(:,:,2),size(X));
hessxx = reshape(hess(:,:,1),size(X));
...
err1 = d1*val(5,:).' - gradx(5,5);
err2 = d1*val(:,5) - grady(5,5);
err3 = d2*val(5,:).' - hessxx(5,5);
...
[val] = surfwave.capillary.gshelm(rts,ejs,src,targ);
[~,grad] = surfwave.capillary.gshelm(rts,ejs,src,targ);
[~,~,hess] = surfwave.capillary.gshelm(rts,ejs,src,targ);
[~,~,~,third] = surfwave.capillary.gshelm(rts,ejs,src,targ);
[~,~,~,~,fourth] = surfwave.capillary.gshelm(rts,ejs,src,targ);
...
[val] = surfwave.capillary.gphiflex(rts,ejs,src,targ);
[~,grad] = surfwave.capillary.gphiflex(rts,ejs,src,targ);
[~,~,hess] = surfwave.capillary.gphiflex(rts,ejs,src,targ);
[~,~,~,third] = surfwave.capillary.gphiflex(rts,ejs,src,targ);
[~,~,~,~,bilap] = surfwave.capillary.gphiflex(rts,ejs,src,targ);
