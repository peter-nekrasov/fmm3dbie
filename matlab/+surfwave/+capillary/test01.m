%% Finite difference test for Green's functions
% Checking that the Green's functions satisfy the following relation:
%       1/2(\alpha \Delta^2 + \beta)G_S + \gamma G_\phi = 0
% away from the diagonal (x \neq y)

h = 0.01;
xs = 1:h:(1+8*h);
[X,Y] = meshgrid(xs);
alpha = 0.5;
beta = -1.5;
gamma = 1;

src = [];
src.r = [0;0];
targ = [];
targ.r = [X(:) Y(:)].';

[rts,ejs] = surfwave.capillary.find_roots(alpha,beta,gamma);
[gs,~,hess] = surfwave.capillary.gshelm(rts,ejs,src.r,targ.r);
gphi = surfwave.capillary.gphihelm(rts,ejs,src.r,targ.r);
lapgs = hess(:,:,1) + hess(:,:,3);

gs = reshape(gs,size(X));
lapgs = reshape(lapgs,size(X));
gphi = reshape(gphi,size(X));

% finite difference stencils
d2 = zeros(9, 1);
d2(1) = -1/560;
d2(2) = 8/315;
d2(3) = -1/5;
d2(4) = 8/5;
d2(5) = -205/72;
d2(6) = 8/5;
d2(7) = -1/5;
d2(8) = 8/315;
d2(9) = -1/560;
d2 = d2 / h^2;

lap = zeros(9,9);
lap(:,5) = d2;
lap(5,:) = lap(5,:) + d2.';

err = abs(0.5*alpha*sum(lap.*gs,'all') + 0.5*beta*gs(5,5) + gamma*gphi(5,5)) / max(abs(gs(:))) 
err2 = abs(0.5*alpha*lapgs(5,5) + 0.5*beta*gs(5,5) + gamma*gphi(5,5)) / max(abs(gs(:)))


